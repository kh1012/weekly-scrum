...
class PerformanceMonitor {
  ...
  logSummary(): void {
    ...
    if (renderMetrics.length > 0) {
      const avgRender = renderMetrics.reduce((sum, m) => sum + m.duration, 0) / renderMetrics.length;
      ...
      console.log(`  16ms 초과: ${renderMetrics.filter(m => m.duration > 16).length}회`);
    }
...
export function createProfilerCallback(componentName: string) {
  return (
    ...
    if (actualDuration > 16) {
      console.warn(
        `⚠️ [Profiler] ${componentName} (${phase}) took ${actualDuration.toFixed(2)}ms`
      );
    } else if (flags.enableDebugMode) {
      console.log(
